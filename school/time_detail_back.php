...
)
{
	...
	$sql_query2="SELECT * FROM free_solt_detail WHERE $col_name='".$col_value."'"; 
	$result2=$conn->query($sql_query2);/* execution of query */
	if($result2->num_rows > 0)
	{
        $sql_query2="DELETE FROM free_solt_detail WHERE $col_name='".$col_value."'"; 
        $result2=$conn->query($sql_query2);/* execution of query */
    }
	
	$sql_statement1=$conn->prepare(" INSERT INTO free_solt_detail ( $col_name )VALUES(?)");
			...
			$time_for_id=str_replace( "." , "_" ,$time_for_id);
			 ...
			 //create a col for timmming
				...
				//statement/query for insert data in time col 
				 $sql_statement2=$conn->prepare(" UPDATE free_solt_detail SET $time_for_id = ? WHERE $col_name= ?;");
